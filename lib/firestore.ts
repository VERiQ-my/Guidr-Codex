...
// Scan actions are intentionally best-effort: a completed verdict must never be blocked by optional account sync.
export async function saveCase(_analysis: Analysis) { void _analysis; }
export async function awardXP(_amount: number) { void _amount; }
export async function incrementStat(_name: 'casesScanned') { void _name; }
export async function incrementScamType(_type: string) { void _type; }



export function subscribeEntitlements(_uid: string, callback: (value: Entitlements) => void) {
  void _uid;
