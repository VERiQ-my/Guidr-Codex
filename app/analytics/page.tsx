...
/** Resolve a case's createdAt (Firestore Timestamp or raw) to a Date or null. */
...
// Where to send people who tap the sponsored slot. The actual provider
// (Gmail / Outlook / Yahoo / default mail app / copy) is picked by the user
// via EmailComposerModal, so we don't bias the link to any one client.
...
// Time ranges for the dashboard. `label` is what the trend card shows.
const RANGES = [
...
export default function AnalyticsPage() {
  ...
  // Render-pure "now": reading Date.now() during render is impure, so we
  // snapshot it in state and refresh it whenever new cases stream in.
  const [now, setNow] = useState(() => Date.now());

  // ── Per-user cases (analytics are scoped to the signed-in account) ──
  useEffect(() => {
    if (!user) return;
  ...
  // ── Range-scoped slices: current period vs the one immediately before it ──
  ...
  // Period-over-period change in cases filed. Up = red (more scams hitting you).
  const trendPct =
    previous.length === 0
      ? current.length > 0
        ? 100
  ...
  // ── Scam-type breakdown for the current range (canonical categories) ──
  // We normalize the raw scam_type so "phishing_attempt" and "Phishing
  // (Payment Info)" collapse into one bucket, then drop the safe "None".
  ...
  // Trending now: the categories you're seeing most this period.
  ...
  // Emerging: categories growing vs the previous period (new ones count as
  // rising). Sorted by growth, capped to a few so it stays a signal.
  ...
  return (
    <div className="guidr-container">
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar px-4 py-5 pb-safe flex flex-col gap-5">
        ...
        {/* ── Stat cards (2×2, scoped to current user + range) ── */}
        ...
        {/* ── Advertisement slot (open — invites brands to inquire) ── */}
        ...
        {/* ── Scam types: trending now + emerging ── */}
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 guidr-animate-in guidr-stagger-4">
          ...
          {/* Emerging — categories rising vs the previous period */}
          {emerging.length > 0 && (
        ...
        {/* ── Latest scam news from around the world ── */}
