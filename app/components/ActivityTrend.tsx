...
/**
 * Risky-encounters-over-time bar chart for the Analytics page.
 *
 * Single series (scam + suspicious cases per bucket), so the card title names
 * the series and no legend is needed. Inline SVG, no chart library — bars are
 * thin with rounded tops anchored to the baseline, 2px gaps, one selective
 * direct label on the peak bucket, sparse axis labels, and a per-bar hover
 * tooltip. Bucketing follows the page's active range: 7 days for a week,
 * six 5-day buckets for a month, 12 months for a year.
 */
...
interface Bucket {
  label: string; // sparse axis label ("" for unlabeled buckets)
...
function buildBuckets(cases: TrendCase[], range: TrendRange, now: number): Bucket[] {
  ...
  if (range === "Year") {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      buckets.push({
        // Label quarters only, so the axis stays recessive.
  ...
  const per = range === "Week" ? 1 : 5; // bucket width in days
  const n = days / per;
  const buckets: Bucket[] = [];
  for (let i = n - 1; i >= 0; i--) {
    ...
    const startD = new Date(start + DAY_MS); // first day inside the bucket
    ...
    if (range === "Week") {
      ...
      buckets.push({
        // First, middle, and last buckets get labels; the rest stay quiet.
...
export default function ActivityTrend({
  ...
  // Fixed viewBox; the SVG scales to the card width.
  const W = 320;
  const H = 96;
  const AXIS_H = 16;
        ...
        <div className="relative">
          ...
          >
            ...
            {buckets.map((b, i) => {
              ...
              return (
                <g key={i}>
                  {/* Rounded top, flat bottom: clip the rounded rect at the baseline. */}
                  <rect
                    x={x}
                    y={y}
                    width={barW}
