...
/**
 * Family-wide risk overview for guardians, shown on Analytics. Merges the
 * guardian's active wards (guardian_links) with the period's alert feed
 * (users/{uid}/guardian_events): every ward gets a row even when quiet, so
 * "no news" is visible good news rather than an empty section. Hidden
 * entirely for users who aren't guardians.
 */
...
export default function WardOverview({ periodMs, now }: { periodMs: number; now: number }) {
  ...
  const wards = useMemo<WardRow[]>(() => {
    ...
    for (const e of events) {
      ...
      // Events can outlive a link (ward removed the guardian) — skip those.
