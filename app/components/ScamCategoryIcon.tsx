/**
 * One place that maps a scam category → icon + color, so Phishing / Job Scam /
 * Investment etc. look identical on My Cases, the analytics leaderboard, and
 * scan results.
 *
 * Input can be a raw model string ("phishing_attempt") or a canonical name —
 * we normalize first, so callers don't have to.
 */

import { normalizeScamType } from "@/lib/scam-categories";

/** Tailwind text + bg pair for each canonical category (icon tile + accents). */
...
/** Text + bg classes for a category (accepts raw or canonical input). */
export function categoryColor(scamType: string | undefined | null) {
  return CATEGORY_COLORS[normalizeScamType(scamType)] || FALLBACK;
}

/** Canonical category name for a (possibly raw) scam type. */
export function categoryName(scamType: string | undefined | null): string {
  return normalizeScamType(scamType);
}

/**
 * User-facing label. Unlike categoryName, this never shows the internal
 * catch-all buckets to people:
 *   - safe verdicts / "None" → "No threat detected"
 *   - "Other"                → "Other scam"
 * Everything else is the canonical category as-is.
 */
...
/* ── Per-category glyphs (24×24, stroke = currentColor so color classes win) ── */
...
/** Inline category icon. Color comes from `className` (e.g. categoryColor().text). */
