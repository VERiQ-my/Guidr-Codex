...
interface EmailComposerModalProps {
  ...
  subject: string;
  body?: string;
  /** Headline shown at the top of the modal. Defaults to "Send email". */
  title?: string;
...
function buildProviderUrl(provider: Provider, to: string, subject: string, body: string): string | null {
  ...
  switch (provider) {
    ...
    case "outlook":
      // outlook.live.com works for personal accounts; users signed into a
      // business tenant get redirected to outlook.office.com automatically.
...
export default function EmailComposerModal({
  ...
  async function handlePick(provider: Provider) {
    if (provider === "copy") {
      // Copy a useful artifact — both the recipient and the body — so the
      // user can paste into any compose window without losing the To: field.
      const text = `To: ${to}\nSubject: ${subject}\n\n${body}`;
      try {
        ...
        // Older browsers / hardened privacy modes: fall back to a prompt so
        // the user can still grab the text instead of leaving them stuck.
