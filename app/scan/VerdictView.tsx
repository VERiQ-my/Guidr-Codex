...
function ReportCard({ title, body, action, icon: Icon, href, urgent, onClick }: ReportCardProps) {
  const className = `rounded-xl border p-4 text-left transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-guidr-primary/15 ${urgent ? "border-red-200 bg-red-50 text-red-800 hover:border-red-400" : "border-black/10 hover:border-guidr-primary hover:bg-guidr-primary/[0.03]"}`;
  const content = <div className="flex items-start gap-3"><span className={`flex size-9 shrink-0 items-center justify-center rounded-full ${urgent ? "bg-white" : "bg-guidr-primary/10 text-guidr-primary"}`}><Icon size={18} /></span><div><strong className="block">{title}</strong><p className={`mt-1 text-sm leading-5 ${urgent ? "" : "text-guidr-muted"}`}>{body}</p><span className={`mt-3 inline-flex items-center gap-1 text-sm font-bold ${urgent ? "" : "text-guidr-primary"}`}>{action}<ArrowUpRight size={15} /></span></div></div>;
  return href ? <a href={href} target={href.startsWith("http") ? "_blank" : undefined} rel={href.startsWith("http") ? "noreferrer" : undefined} className={className}>{content}</a> : <button type="button" onClick={onClick} className={className}>{content}</button>;
}
...
export default function VerdictView({ analysis }: { analysis: Analysis }) {
  ...
  return <section className="scan-panel overflow-hidden">
    ...
    <div className="p-5 sm:p-6">
      ...
      {analysis.verdict !== "LIKELY_SAFE" && <section className="mt-7 border-t border-black/5 pt-6" aria-labelledby="report-options"><h2 id="report-options" className="font-bold">Need to report this?</h2><p className="mt-1 text-sm text-guidr-muted">If money was sent, act quickly. Otherwise, preserve the evidence and use the relevant reporting channel.</p><div className="mt-4"><ReportCard urgent title="Money was sent" body="Call NSRC at 997 now, then call your bank." action="Call 997" icon={PhoneCall} href="tel:997" /></div><div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm font-semibold text-guidr-primary"><a className="hover:underline underline-offset-4" href="https://aduan.mcmc.gov.my/" target="_blank" rel="noreferrer">Report a message to MCMC</a><a className="hover:underline underline-offset-4" href="https://www.rmp.gov.my/" target="_blank" rel="noreferrer">Make a police report</a></div></section>}
